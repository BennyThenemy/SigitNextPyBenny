...
def main():
    ...
    for i in range(0, len(first) - 3, 2):
        start = (first[i], first[i+1])
        end = (first[i+2], first[i+3])
        draw.line((start, end), fill="black", width=3)

    for i in range(0, len(second) - 3, 2):
        start = (second[i], second[i+1])
        end = (second[i+2], second[i+3])
        draw.line((start, end), fill="black", width=3)

    # adding text:
    text = 'Its a bull!!! (or cow)'
    font_size = 20
    font = ImageFont.truetype('arial.ttf', font_size)
    text_position = (50, 50)
    draw.text(text_position, text, fill='red', font=font)

    # saving the picture as 'ex6p4answer.jpg':
    image.save('ex6p4answer.jpg')
